...
if ($employee->id==$id && $employee->user_role=='super admin') {
...
'view_access'=>$view,'edit_access'=>$edit,'full_access'=>$full]);
            ...
            return view('Admin.admin_permistion', compact('adminroles', 'user'));
    } 
...
if(auth::user()->user_role=='super admin' || auth::user()->user_role=='admin'){
   ...
   public function RolesEdit($id)
   {
            ...
            $user=User::select('id','password','first_name','last_name','email','photo','user_role','dob',)->where('id',$id)->first();

     return view('Admin.admin_permistion',get_defined_vars());
